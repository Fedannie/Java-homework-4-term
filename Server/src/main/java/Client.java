...
class Client implements AutoCloseable {
    ...
    /**
     * Construct {@code Client} object by socket to connect to server.
     ...
     */
    Client(Socket socket) throws DataStreamException {
        this.socket = socket;
        try {
            ...
            try {
                ...
                try{
                    ...
                    throw new DataStreamException("Error when handling streams in Client constructor");
                }
                throw new DataStreamException("Error when handling streams in Client constructor");
            }
            throw new DataStreamException("Error when handling streams in Client constructor");
     ...
     * @throws DataStreamException if there is an error with using {@code DataInputStream} or {@code DataOutputStream}
     */
    Element[] list(String path) throws DataStreamException {
        try {
            out.writeInt(1);
            out.writeUTF(path);
            ...
            throw new DataStreamException("Error when handling streams in Client.list");
        }
    }
     ...
     * @throws DataStreamException if there is an error with using {@code DataInputStream} or {@code DataOutputStream}
     */
    byte[] get(String path) throws DataStreamException {
        try {
            out.writeInt(2);
            out.writeUTF(path);
            ...
            throw new DataStreamException("Error when handling streams in Client.get");
        }
    }
    ...
    public void close() throws DataStreamException, SocketException {
        try {
            ...
            throw new DataStreamException("Error when handling streams in Client.close");
        }
        try {
            ...
            throw new SocketException("Error when trying to shut down a Socket object in Client.close");
        }
    }
}
